            ...
            {filtered.map(inv => (
              <div key={inv.id} className="card" style={{ padding: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                  <div>
                    <code style={{ fontSize: '0.78rem', color: 'var(--gold)' }}>{inv.id.slice(0, 8).toUpperCase()}</code>
                    <div style={{ fontWeight: 700, fontSize: '1.05rem', color: 'var(--gold)', marginTop: 4 }}>LKR {(inv.amount || 0).toLocaleString()}</div>
                  </div>
                  <span className={`badge ${inv.whatsappSent ? 'badge-success' : 'badge-muted'}`}>{inv.whatsappSent ? '✓ Sent' : 'Not sent'}</span>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center', marginBottom: '0.6rem', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                  <span>Booking: <a href={`/admin/bookings/${inv.bookingId}`} style={{ color: 'var(--text-primary)', textDecoration: 'underline' }}>{inv.bookingId?.slice(0, 8).toUpperCase()}</a></span>
                  <span>·</span>
